...
class RoundTouchDisplay:
    def __init__(self):
        ...
        # Cabinets aim with a mouse (or a stick mapped to the pointer), so the
        # kiosk cursor hiding is opt-out here; touch builds keep it hidden.
        try:
    ...
    def _radar_swipe_committed(self, swipe_start, swipe_end) -> bool:
        ...
        if swipe_start is None and swipe_end is None:
            # Injected by a cabinet button: no path to measure, and the
            # short-flick branch below needs coordinates it will never get.
            return True
